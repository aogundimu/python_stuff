...
def answer_three():
    ...
    """
    Column names for result =  ['Energy Supply' 'Energy Supply per Capita' "% Renewable's" 
    'GDP 2006' 'GDP 2007' 'GDP 2008' 'GDP 2009' 'GDP 2010' 'GDP 2011' 'GDP 2012'
...
def answer_six():
    ...
    country = Top15.loc[Top15["% Renewable's"].idxmax()].name
...
def answer_ten():
    ...
    Top15 = Top15.sort_values("% Renewable's", ascending=True)
...
def answer_eleven():
    ...
    #print( Top15.set_index('Continent').groupby(level=0)['Population']
    #       .aggregate({'size': len, 'sum': np.sum, 'mean': np.mean, 'std': np.std}) )
