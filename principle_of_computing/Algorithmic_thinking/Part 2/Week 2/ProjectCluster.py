...
import math
import alg_cluster.Cluster as Cluster
...
def hierarchical_clustering(cluster_list, num_clusters):
    ...
    while len(clusters_c) > num_clusters:
        result = (float('inf'), -1, -1)
        for index_i in range(0, len(clusters_c)):
            for index_j in range(0, len(clusters_c)):
                if index_i != index_j:
                    distance = clusters_c[index_i].distance(clusters_c[index_j])
                    if ( distance < result[0] ):
                        result = (distance, index_i, index_j)
